    ...
    <div style={{minHeight:"100vh",background:BG,color:TEXT,padding:"2rem 1rem"}}>
      <div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gap:14}}>
          {[
            {p:"explorer" as Phase,emoji:"🎨",t:"Explorer les 7 Patterns",d:"Singleton, Factory, Builder, Adapter, Composite, Observer, Strategy",c:BLUE},
            {p:"match" as Phase,emoji:"🎯",t:"Pattern → Scénario",d:"Associez chaque pattern à son cas d'usage réel",c:ORANGE},
            {p:"quiz" as Phase,emoji:"🧠",t:"Quiz Design Patterns",d:"8 questions pour maîtriser les patterns",c:GREEN},
          ].map(g=>(
            ...
            {filtered.map(p=>(
              <div key={p.name} style={{border:`1px solid ${expanded===p.name?catCol[p.cat]:BORDER}`,borderRadius:10,overflow:"hidden"}}>
                <button onClick={()=>setExpanded(expanded===p.name?null:p.name)} style={{width:"100%",padding:"12px 16px",background:expanded===p.name?catCol[p.cat]+"15":CARD,border:"none",cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><span style={{fontSize:20,marginRight:8}}>{p.emoji}</span><span style={{fontWeight:600,fontSize:15}}>{p.name}</span><span style={{marginLeft:8,fontSize:11,padding:"2px 8px",borderRadius:10,background:catCol[p.cat]+"30",color:catCol[p.cat],fontWeight:600}}>{p.cat}</span></div>
